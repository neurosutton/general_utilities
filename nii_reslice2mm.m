...
[filepath,name,ext] = spm_fileparts(fnm); %strip volume, e.g. 'img.nii,1'->img.nii
fnm = fullfile(filepath, [ name, ext]);
inhdr = spm_vol(fnm);
...
fnm = fullfile(filepath, ['twomm_', name, ext]);
outhdr.fname = fnm;
